...
#include <math.h>
#include "bsp_mathacl.h"  /* MATHACL 硬件加速三角函数 */
...
void app_cf_init(const app_cf_config_t *cfg)
{
    ...
    if (s_cfg.wheel_radius_m <= 0.0f) { s_cfg.wheel_radius_m = PRJ_CF_WHEEL_RADIUS_M; }
    if (s_cfg.wheel_base_m <= 0.0f) { s_cfg.wheel_base_m = PRJ_CF_WHEEL_BASE_M; }
                   ...
                   float imu_yaw_deg, float dt_s)
{
    ...
    s_y += s_vx * mathacl_sinf(s_heading_rad) * dt_s;
}
