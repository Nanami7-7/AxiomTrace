...
2. 应用层现已统一调用 `bsp_motor_*`。`bsp_motor.c/h` 是公共门面，默认分发到DRV8870；TB6612由独立 `bsp_tb6612.c/h` 作为备用后端保留，并通过编译期宏与DRV8870互斥选择。
...
| `Application/app_main.c`、`Task/task_control.c`、`app_vofa.c` | 主应用路径已使用统一 `bsp_motor` 门面 | RTOS 下保护响应的硬实时性 |
| `BSP/Peripherals/bsp_motor.[ch]`、`bsp_tb6612.[ch]` | 统一门面和TB6612备用后端已分离，编译期单选 | TB6612在备用硬件上的实测结果 |
| 当前 `Config/ti_msp_dl_config.c` | 可见 `.startTimer = DL_TIMER_START`，各 CC 初值为 500 | 上电首脉冲必然安全；必须示波器验证 |

> 当前 `Config/empty.syscfg` 是DRV8870板级配置。双后端分层不盲目修改该文件；TB6612应使用独立SysConfig profile。
...
当前不再把 `bsp_motor.c/h` 当作TB6612芯片驱动，而是将其重构为统一门面：

```mermaid
flowchart TB
    APP["应用/PID/运动规划/协议"] --> SVC["bsp_motor\n统一编号、命令、停止语义"]
    SVC --> CAPS["capabilities\nPower/Coast/Brake/LAP/DIR"]
    SVC --> TB["bsp_tb6612 backend\nPWM + IN1/IN2 + optional STBY"]
    SVC --> LAP["bsp_drv8870 backend\n单PWM锁相 + PB19"]
```
...
应用层普通控制只包含 `bsp_motor.h`。DRV8870示波器FactoryTest仍允许诊断模块直接调用 `bsp_drv8870_hw_scope_*()`，这是明确隔离的芯片专属边界。

### 8.2 当前公共能力语义

公共能力位可查询功率闸门、真实Coast、真实Brake、锁相驱动和方向GPIO：

- 当前DRV8870：有PB19功率门控、有锁相驱动，不具备真实Coast/Brake；
- TB6612：具备真实Coast/Brake和方向GPIO；仅配置STBY软件控制时才声明真实功率闸门。

为了兼容现有业务，公共 `bsp_motor_stop()` 在DRV8870后端会将COAST/BRAKE安全降级为50%中性，并通过能力位明确暴露限制。若未来业务必须保证特定停止物理状态，应在调用前检查能力位，或升级API为可返回 `BSP_ERR_UNSUPPORTED` 的严格停止请求。

### 8.3 TB6612安全实现

TB6612后端已加入：

- PWM通道、方向GPIO、重复资源、方向符号和可选STBY配置校验；
- 统一 `±500` 命令到 `0..period` 的线性映射；
- 正确高阻Coast：`IN1=IN2=0、PWM=100%`；
- 短制动Brake：PWM低；
- 跨方向时先进入至少一个上层控制调用周期的高阻Coast；
- GPIO/PWM写入失败后执行安全关闭；
- 当前SysConfig缺少方向GPIO时主动 `#error`，禁止生成错误固件。

TB6612仍未完成备用硬件实测，不能标记为生产验证通过。

### 8.4 当前切换约束与后续阶段

| 阶段 | 当前状态 | 验收 | 回滚 |
|---|---|---|---|
| R1 统一门面和capability | 已实施 | 默认DRV8870两个Target构建通过 | `git revert`本次分层提交 |
| R2 应用层迁移 | 已实施 | 普通控制无直接芯片后端依赖 | 恢复迁移前安全分支 |
| R3 TB6612编译门禁 | 已实施，待记录门禁构建 | 仅出现预期缺GPIO错误 | 恢复默认后端宏 |
| R4 TB6612独立SysConfig与硬件验证 | 未实施 | 无电机波形、单轮、四轮、失联与换向验收 | 恢复DRV8870 SysConfig和固件 |
| R5 斜坡、跨零时间状态机、故障锁存 | 后续安全改进 | 实时性和故障注入验收 | feature flag/独立提交回退 |

禁止使用 `git reset --hard` 清除现场记录；软件回滚优先使用安全分支、独立提交和 `git revert <commit>`。

完整切换、历史引脚、真值表、门禁和验收流程见《电机驱动双后端分层与切换指南.md》。
