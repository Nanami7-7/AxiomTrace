/**
 * @file    bsp_ble_uart.h
 * @brief   外部 BLE 模块使用的 UART1 字节流板级驱动接口。
 * @details
 * 本模块只负责 UART1 的接收中断、软件环形缓冲和轮询发送，不解析 JDY-23
 * AT 命令，也不参与 UART0 调试串口或 DMA 状态管理。
 * @note 默认硬件映射为 TX=PB6、RX=PB7、9600 8N1，具体引脚和波特率以 SysConfig
 *       及 project_config.h 的实际配置为准。
 */
...
/**
 * @brief 丢弃 UART1 软件接收缓冲区中的全部数据。
 */
void bsp_ble_uart_flush_rx(void);

/**
 * @brief 获取 UART1 BLE 诊断计数的原子快照。
...
/**
 * @brief UART1 中断服务入口。
 * @details 由对应的 UART1 IRQHandler 调用；正常上下文中不应直接轮询调用。
 */
