/**
 * @file    proto_uart1_b.c
 * @brief   Board B UART1 板间协议传输层实现。
 */
...
static uint8_t s_rx_storage[PROTO_UART1_B_RX_BUF_SIZE];
static bsp_ringbuf_t s_rx_ring;
static volatile proto_uart1_b_diag_t s_diag;
...
bsp_status_t proto_uart1_b_init(void)
{
    ...
    bsp_ringbuf_init(&s_rx_ring, s_rx_storage, PROTO_UART1_B_RX_BUF_SIZE);
    while (DL_UART_receiveDataCheck(UART_1_INST, &stale)) {
    }
    /* 打开 UART1 外设 RX 中断，再打开对应 NVIC。 */
    DL_UART_Main_enableInterrupt(UART_1_INST, DL_UART_MAIN_INTERRUPT_RX);
    if (hal_uart_enable_irq(HAL_UART_1) != HAL_OK) {
...
void proto_uart1_b_deinit(void)
{
    ...
    (void)hal_uart_disable_irq(HAL_UART_1);
...
bsp_status_t proto_uart1_b_write(const uint8_t *data, uint16_t len)
{
    ...
    return map_hal_status(hal_uart_transmit_buf(HAL_UART_1, data, len));
}
...
void proto_uart1_b_irq_handler(void)
{
    ...
    case DL_UART_IIDX_RX_TIMEOUT_ERROR:
        {
            uint8_t data;
            while (DL_UART_receiveDataCheck(UART_1_INST, &data)) {
