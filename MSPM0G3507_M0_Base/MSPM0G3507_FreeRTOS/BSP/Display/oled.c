...
void OLED_Init() {
  ...
  OLED_SendCmd(0xD5); OLED_SendCmd(0x80); /* display clock */
  OLED_SendCmd(0xA8); OLED_SendCmd(0x3F); /* multiplex ratio: 1/64 */
  OLED_SendCmd(0xD3); OLED_SendCmd(0x00); /* display offset */
  OLED_SendCmd(0x40);                    /* start line 0 */
  OLED_SendCmd(0x8D); OLED_SendCmd(0x14); /* internal charge pump */
  OLED_SendCmd(0x20); OLED_SendCmd(0x02); /* page addressing mode */
  OLED_SendCmd(0xA1);                    /* segment remap */
  OLED_SendCmd(0xC8);                    /* COM scan direction */
  OLED_SendCmd(0xDA); OLED_SendCmd(0x12); /* COM pins */
  OLED_SendCmd(0x81); OLED_SendCmd(0xCF); /* contrast */
  OLED_SendCmd(0xD9); OLED_SendCmd(0xF1); /* pre-charge */
  OLED_SendCmd(0xDB); OLED_SendCmd(0x40); /* VCOMH */
  OLED_SendCmd(0xA4);                    /* display follows RAM */
...
void OLED_DisPlay_On() {
  OLED_SendCmd(0x8D); /* 中文注释：OLED 显示驱动。 */
...
void OLED_DisPlay_Off() {
  OLED_SendCmd(0x8D); /* 中文注释：OLED 显示驱动。 */
  OLED_SendCmd(0x10); /* 中文注释：OLED 显示驱动。 */
  OLED_SendCmd(0xAE); // 关闭屏幕
}

/**
 * @brief   OLED 显示驱动。
 * @param  ColorMode 参数说明。
 * @note   使用时请遵循接口约束。
 */
...
void OLED_ShowFrame() {
  ...
  for (uint8_t i = 0; i < OLED_PAGE; i++) {
    ...
    OLED_SendCmd(0x02);     /* 中文注释：OLED 显示驱动。 */
    OLED_SendCmd(0x10);     /* 中文注释：OLED 显示驱动。 */
...
void OLED_SetBlock(uint8_t x, uint8_t y, const uint8_t *data, uint8_t w, uint8_t h, OLED_ColorMode color) {
  uint8_t fullRow = h / 8; /* 中文注释：OLED 显示驱动。 */
...
void OLED_PrintString(uint8_t x, uint8_t y, const char *str, const Font *font, OLED_ColorMode color) {
  uint16_t i = 0;                                       /* 中文注释：OLED 显示驱动。 */
  uint8_t oneLen = (((font->h + 7) / 8) * font->w) + 4; // 一个字模占多少字节
  uint8_t found;                                        // 是否找到字模
  uint8_t utf8Len;                                      // UTF-8编码长度
  uint8_t *head;                                        /* 中文注释：OLED 显示驱动。 */
  while (str[i]) {
    found = 0;
