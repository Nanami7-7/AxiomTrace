...
/* 描述硬件和内存使用情况的常量 */
/* ⚠️ 此值必须与 ti_msp_dl_config.h 中 CPUCLK_FREQ 保持一致。
 *    不直接 #include ti_msp_dl_config.h，因为会拉入设备寄存器头文件
 *    (mspm0g350x.h)，其中的 MATHACL_BASE/MATHACL 宏会与 bsp_mathacl.h
 *    冲突。修改 sysconfig 时钟配置后需手动同步此值。 */
#define configCPU_CLOCK_HZ ((unsigned long) 80000000)  // CPU时钟频率（Hz）
/* 允许的最小栈大小（以字为单位） */
#define configMINIMAL_STACK_SIZE ((unsigned short) 128)
#define configMAX_TASK_NAME_LEN (12)                   // 任务名称的最大长度（字符数）
#define configTOTAL_HEAP_SIZE ((size_t)(14* 1024))     // 总堆大小（字节）
...
#define configUSE_RECURSIVE_MUTEXES 0                  // 未使用递归互斥，关闭以节省 Flash
/*
