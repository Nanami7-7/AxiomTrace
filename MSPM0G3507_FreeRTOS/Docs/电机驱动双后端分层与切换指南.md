...
统一门面内部为保持零运行时开销，会把公共 `bsp_motor_id_t` 转换为所选后端ID。`bsp_motor.c` 使用 `_Static_assert` 固化 A~D 和通道数量映射；以后若调整任一枚举顺序，构建必须直接失败，禁止静默形成电机错通道。

DRV8870 原始 compare/示波器接口是唯一允许的应用层后端特例，并且只在 `PRJ_DRV8870_FACTORY_TEST_ENABLE != 0U` 时包含 `bsp_drv8870.h`。普通生产Target的 `app_debug` 依赖中不应出现该专用头文件。
...
| 公共ID与两个后端ID的编译期映射契约 | 已通过：A~D及通道数量由 `_Static_assert` 固化 |
| 生产Target隔离DRV8870专用诊断头文件 | 已通过：普通 `app_debug.d` 仅依赖 `bsp_motor.h`；FactoryTest才依赖 `bsp_drv8870.h` |
