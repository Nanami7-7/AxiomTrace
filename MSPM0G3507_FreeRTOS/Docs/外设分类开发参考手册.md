...
- `BSP/Peripherals/bsp_drv8870.h/.c`：电机语义和安全联锁
...
生产控制应优先使用 `bsp_drv8870_*`。只有做底层波形验证时才直接调用HAL：
...
> 直接写比较寄存器不会自动执行PB19电源联锁、方向修正和停机中性处理。生产代码不要绕过DRV8870 BSP。
...
- `BSP/Peripherals/bsp_drv8870.h/.c`：校验、功率联锁和死区分段映射；
- `Application/Task/task_control.c`：将LF/LB/RF/RB编码器顺序重排为A/B/C/D电机顺序；
- `Application/app_main.c`：板级配置实例、PID输出限幅和配置一致性检查。
...
| `PRJ_DRV8870_SPEED_COMMAND_MAX` | 500 | 由`PWM_PERIOD/2`派生，不单独手改 |
...
| API | 参数范围 | 说明 |
|---|---|---|
| `bsp_drv8870_init(cfg,count,timer,period)` | `cfg != NULL`；`count=1~4`；`period>=2` | 校验配置、四路置中性、PB19保持OFF |
| `bsp_drv8870_power_enable()` | 无 | 先置中性，再拉高PB19；不包含阻塞延时 |
| `bsp_drv8870_power_disable()` | 无 | 先置中性，再拉低PB19 |
| `bsp_drv8870_power_is_enabled()` | 无 | 返回软件状态和GPIO输出锁存状态，不代表物理12V反馈 |
| `bsp_drv8870_set_speed(motor,speed)` | `motor=A~D`；内部限幅`-500~+500` | 执行安装方向和死区补偿；PB19未开启返回`BSP_ERR_NOT_READY` |
| `bsp_drv8870_stop(motor,mode)` | 合法电机ID；任一停止枚举 | 当前硬件均写入中性点，不是真Coast/Brake |
| `bsp_drv8870_stop_all()` | 无 | 四路写中性，不自动关闭PB19 |
| `bsp_drv8870_get_duty_max()` | 无 | 初始化后返回有符号命令最大绝对值，当前500 |
| `bsp_drv8870_percent_to_absolute_duty(percent)` | `0~100`，超出按100钳位 | 原始绝对占空比转换，不执行死区补偿 |
...
void motor_demo(void)
{
    ...
    /* +150是车体前进命令；驱动会自动映射到55%以上。 */
    if (bsp_drv8870_set_speed(BSP_DRV8870_A, +150) != BSP_OK) {
...
应用层若需要“速度百分比”，应先转换为有符号命令：

```c
int32_t speed = (int32_t)(percent * bsp_drv8870_get_duty_max() / 100U);
(void)bsp_drv8870_set_speed(BSP_DRV8870_A, speed);
```
...
该接口绕过业务死区映射，只用于悬空车轮、限流电源和示波器测量。生产控制必须使用`bsp_drv8870_set_speed()`。
...
- `BSP/Peripherals/bsp_motor.h`
- `BSP/Peripherals/bsp_motor.c`
- `Config/project_config.h` 的 `#ifdef BSP_MOTOR_ENABLE` 区域

它是回退/兼容实现，不是当前生产电机架构。当前PWM和引脚已重构为DRV8870映射，旧注释中的PB8、PA22、PA15、PA17以及IN1/IN2映射可能与现板不一致。

### 8.2 兼容API参考

这些声明只有定义 `BSP_MOTOR_ENABLE` 后才可见：

| API | 参数范围 | 说明 |
|---|---|---|
| `bsp_motor_init(cfg,count,timer,period)` | `cfg != NULL`；`count`为1～4；`timer`应为PWM实例；`period>0` | 初始化IN1/IN2并启动PWM |
| `bsp_motor_get_duty_max()` | 无 | 返回初始化PWM周期；未初始化返回0 |
| `bsp_motor_percent_to_duty(percent)` | 0～100，超出值需以实现为准 | 百分比转0～period占空比 |
| `bsp_motor_set_speed(motor,speed)` | `motor`为A～D；`speed`为`-period`～`+period`，实现会限幅 | 正/反转；0按驱动实现进入停止状态 |
| `bsp_motor_stop(motor,mode)` | `mode`为`BSP_MOTOR_MODE_COAST/BRAKE` | 独立滑行或刹车 |
| `bsp_motor_stop_all()` | 无 | 四路刹车停止 |

配置结构包含 `pwm_ch`、IN1端口/引脚、IN2端口/引脚和 `dir_sign`。`pwm_ch`合法范围0～3，`dir_sign`使用+1/-1，0按+1处理。

### 8.3 恢复与调用教程（仅适用于真实TB6612硬件）

1. 从已验证Git标签建立独立分支；
2. 根据原理图在 `empty.syscfg` 恢复四路PWM和八路IN1/IN2 GPIO；
3. 在 `project_config.h` 或Keil宏中定义 `BSP_MOTOR_ENABLE`；
4. 核对 `PRJ_MOTOR_CONFIGS` 中每个生成引脚宏真实存在；
5. 确认DRV8870 BSP不再初始化或控制TIMA0；
6. 先不接电机，用逻辑分析仪验证方向和PWM；
7. 再用限流电源和悬空电机低占空比验证。
...
static const bsp_motor_config_t s_tb6612_cfg[BSP_MOTOR_COUNT] =
    PRJ_MOTOR_CONFIGS;

int tb6612_demo_init(void)
{
    return (int)bsp_motor_init(s_tb6612_cfg,
                               BSP_MOTOR_COUNT,
                               PRJ_MOTOR_PWM_TIMER,
                               PRJ_MOTOR_PWM_PERIOD);
}

void tb6612_demo_run_and_stop(void)
{
    uint32_t duty = bsp_motor_percent_to_duty(20U);
    (void)bsp_motor_set_speed(BSP_MOTOR_A, (int32_t)duty);
    (void)bsp_motor_stop(BSP_MOTOR_A, BSP_MOTOR_MODE_COAST);
}
#endif
```
...
1. 新代码使用 `bsp_drv8870_*`；
2. 只有明确恢复TB6612硬件和SysConfig GPIO后才定义 `BSP_MOTOR_ENABLE`；
3. TB6612和DRV8870不能同时拥有TIMA0及同一组PWM通道；
4. 切换驱动前应建立独立Git标签，并逐项核对PWM、IN1/IN2、STBY和电源极性；
5. 上述示例在当前DRV8870板上不得启用。
