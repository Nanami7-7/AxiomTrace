...
> 版本：v1.1  |  状态：**已冻结**  |  对应代码：`baremetal/core/axiom_event.c`, `baremetal/core/axiom_crc.c`
...
wire v1.1 保持 8 字节报头不变，并通过类型化 payload 字段承载主机解码元数据：
...
如果解码器在有效载荷中遇到无法识别的类型标签（剩余保留范围 `0x0C-0x7F`）：
...
当前版本：**0x11** (v1.1)。主机解码器继续能够解析旧的 v1.0 frame body。
