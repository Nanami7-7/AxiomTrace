...
| 字段 | 类型 | 必选 | 描述 |
|------|------|------|------|
| `size` | `uint16_t` | 自动 | 编译时结构体大小。由 `AXIOM_BACKEND_INIT()` 自动填充。旧的零初始化后端（size==0）视为旧版布局。 |
...
| `axiom_backend_usb.c` | USB CDC | v1.x 计划项；不阻断 v1.0 发布 |
| `axiom_backend_rtt.c` | RTT | `SEGGER_RTT_WriteBlock()` 或封装 |
| `axiom_backend_swo.c` | SWO/ITM | v1.x 计划项；不阻断 v1.0 发布 |
| `axiom_backend_canfd.c` | CAN-FD | v1.x 计划项；不阻断 v1.0 发布 |
| `axiom_capsule.c` | Flash capsule | `axiom_port_flash_erase()` / `axiom_port_flash_write()` / `axiom_port_flash_read()` |

v1.0 发布支持的 transport 为 Memory、UART、RTT 与 generic host port。USB CDC、SWO/ITM 和 CAN-FD 保留为 v1.x 扩展目标。
