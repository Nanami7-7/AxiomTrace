...
> 版本：v1.1  |  状态：**已冻结**  |  对应代码：`baremetal/core/axiom_event.h`
...
| 0x0A     | 定位元数据   | 5 或 7   | 模式字节及固定宽度源码定位字段    |
| 0x0B     | 元数据身份   | 8        | bundle metadata ID 字节           |
| 0x0C-0x7F| 保留         | —        | 预留给未来标准类型                |
| 0x80-0xFF| 用户定义     | —        | 供项目特定类型使用                |

`定位元数据` 不改变固定报头，只在启用时附加到类型化 payload：
...
3. 后续在剩余 `0x0C-0x7F` 标准范围内新增类型标签时，需要同步更新规范、解码器、Golden Test 以及文档。
...
当前有线格式版本：**v1.1** (`version = 0x11`)。解码器继续支持不含定位元数据的 v1.0 帧结构。
