...
AxiomTrace 采用有界二进制帧组装来降低热路径延迟：

1. **Packed Payload 编码**：前端宏只编码 dictionary 定义的二进制值；固件事件流不写入格式字符串。
...
- **栈上界**：默认短临界区模式使用有界帧缓冲（`AXIOM_MAX_FRAME_LEN`）来降低中断延迟；极端栈受限目标可设置 `AXIOM_SHORT_CS=0`。
